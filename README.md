...
**Research protocol candidate:** open [`research.html`](research.html) for the separate EchoPulse Research v1 route. It uses binocular near vision at a physically measured 40 cm, then right and left monocular distance vision at a physically measured 2 m. Five randomized tumbling E presentations at each size use the published 4/5 staircase. The user rechecks a physical 50 mm ruler calibration, distance, glasses, screen height and lighting before every stage. Distance stages ask a helper to hold the screen and enter the subject's spoken answers. If a correctly sized symbol or its crowding box does not fit on a device, the stage is recorded as unmeasurable rather than being assigned a vision threshold. Passing a level before the next level is blocked by the display is reported descriptively but does not become a measured threshold.

The methodology panel publicly shows the **proposed design priority weights**: 40% protocol fidelity (Vision at Home), 30% measurement controls (Vision at Home and WHOeyes), 20% repeatability and reference comparisons (Peek Acuity and ETDRS), and 10% usability (WHO guidance). These are design decisions, **not statistical pooling of evidence, accuracy estimates, or participant scores**; WHOeyes built on V@home, so counting both as independent proof of the same algorithm would overstate the evidence. Optional second rounds record repeat measurements for descriptive comparison. An explicit download button exports per-letter trials, chosen calibration and a generated examination ID as JSON; no name is included, but the ID could be linked to a participant if the file is shared. Data remains in page memory otherwise. The site does not calculate a clinical test-retest reliability coefficient, compare against a clinician's ETDRS assessment, infer a glasses prescription, or confirm distance by camera. The separate [`vision.html`](vision.html) retains the original five-stage exploratory exercise, including its optional unvalidated camera distance-change cue.
...
These are primary validation studies or official clinical guidance for **comparison and study design**, not evidence that EchoPulse has itself been validated. A different optotype, device, distance, score, user population, or testing protocol cannot inherit published accuracy estimates without a separate comparison study.

- [Vision at Home development and validation (TVST, 2019)](https://tvst.arvojournals.org/article.aspx?articleid=2748706): predecessor for a smartphone tumbling-E visual acuity procedure. Compare test sequence and scoring with the full publication, not just the app name.
- [WHOeyes real-world automatic distance calibration (British Journal of Ophthalmology, 2024)](https://bjo.bmj.com/content/108/11/1613): compare measured viewing distance and implementation; our face-width cue is **not** the validated WHOeyes method.
- [Peek Acuity validation (JAMA Ophthalmology, 2015)](https://pubmed.ncbi.nlm.nih.gov/26022921/): example of evaluating a separate smartphone acuity test against ETDRS and Snellen, including repeatability; its reported accuracy does not transfer to EchoPulse.
- [WHOeyes community screening validity and reliability study (Indian Journal of Community Medicine, 2026)](https://pubmed.ncbi.nlm.nih.gov/42266518/): additional study comparing WHOeyes and a conventional chart in community screening, under its own population and protocol.
- [ETDRS chart repeatability and agreement (JAMA Ophthalmology, 2018)](https://jamanetwork.com/journals/jamaophthalmology/fullarticle/2668281): example of analyzing agreement and repeatability between different optotypes, not a claim of equivalence to a phone screen.
- [National Eye Institute — adult vision screening guidance](https://www.nei.nih.gov/about/education-and-outreach/glaucoma-resources/national-eye-institute-statement-detection-glaucoma-and-adult-vision-screening): distinguish screening each eye from a complete professional examination.
